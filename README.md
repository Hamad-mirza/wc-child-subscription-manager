...
A WooCommerce plugin that lets parents register. Parents manage their own children from a front-end page, admins can manage children on behalf of any parent.
...
The plugin registers a **Children** custom post type, each child owned by the parent's user account. Parents add and edit their own children from a front-end Children page,Every view and selection is scoped to the logged-in parent, so one parent can never see or bind another parent's children.
...
4. A **Children** section appears in the admin, and the front-end Children page.

## Usage

**As a parent:** open the Children page → add or edit your children.
...
- `includes/class-frontend.php` — the front-end Children management page, scoped to the current user.
